...
class opParams:
  def __init__(self):
    ...
    self.default_params = {'camera_offset': {'default': 0.06, 'clip': [-0.5, 0.5], 'allowed_types': [float], 'live': True},
                           ...
                           'kp_0': {'default': 1.0, 'clip': [0.0, 5.0], 'allowed_types': [float], 'live': True},
                           'kp_11': {'default': 1.3, 'clip': [0.0, 5.0], 'allowed_types': [float], 'live': True},
                           'kp_22': {'default': 1.23, 'clip': [0.0, 5.0], 'allowed_types': [float], 'live': True},
                           'kp_45': {'default': 1.1, 'clip': [0.0, 5.0], 'allowed_types': [float], 'live': True},
                           'kp_78': {'default': 0.9, 'clip': [0.0, 5.0], 'allowed_types': [float], 'live': True},
                           'ki_0': {'default': 0.28, 'clip': [0.0, 1.0], 'allowed_types': [float], 'live': True},
                           'ki_78': {'default': 0.22, 'clip': [0.0, 1.0], 'allowed_types': [float], 'live': True},
  ...
  def key_info(self, key):
    ...
    if key in self.default_params:
      ...
      if 'clip' in self.default_params[key]:
        ...
        assert type(c[0]) in key_info.allowed_types, f'clip {c[0]} not in allowed types'
        assert type(c[1]) in key_info.allowed_types, f'clip {c[1]} not in allowed types'
      ...
      if 'default' in self.default_params[key]:
        ...
        if key_info.has_clip:
          assert key_info.default == clip(key_info.default, key_info.min, key_info.max), \
